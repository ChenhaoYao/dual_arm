...
class VrPoseToMoveGroup(Node):
    def __init__(self):
        ...
        self.get_logger().info(
            "VR MoveGroup bridge ready: relative /vr/*/pose -> /move_action pose goals"
        )
    ...
    def _on_pose(self, arm: ArmState, msg: PoseStamped):
        if not arm.config.enabled:
            return
        if arm.goal_active or self.move_group_busy:
            return
        now = self.get_clock().now().nanoseconds * 1e-9
        if now - arm.last_goal_time < self.goal_period:
            return
        ...
        if target is None or not self._target_changed(arm.last_target, target):
            return
    ...
    def _send_goal(self, arm: ArmState, target_pose: Pose):
        stamped = PoseStamped()
        ...
        position_constraint.header = stamped.header
        position_constraint.link_name = arm.config.ee_link
        ...
        orientation_constraint.header = stamped.header
        orientation_constraint.link_name = arm.config.ee_link
        ...
        orientation_constraint.weight = 0.2 if self.follow_orientation else 0.01
        ...
        request.group_name = arm.config.group
        ...
        arm.last_target = target_pose
        future = self.move_group_client.send_goal_async(goal)
        future.add_done_callback(lambda done, state=arm: self._on_goal_response(state, done))
        self.get_logger().info(
            f"{arm.config.name} MoveGroup target: "
            f"x={target_pose.position.x:.3f} y={target_pose.position.y:.3f} z={target_pose.position.z:.3f}"
        )

    def _on_goal_response(self, arm: ArmState, future):
        try:
            goal_handle = future.result()
        except Exception as exc:
            arm.goal_active = False
            self.move_group_busy = False
            self.get_logger().warn(f"{arm.config.name} MoveGroup goal request failed: {exc}")
            return
        if goal_handle is None or not goal_handle.accepted:
            arm.goal_active = False
            self.move_group_busy = False
            self.get_logger().warn(f"{arm.config.name} MoveGroup goal rejected")
            return
        result_future = goal_handle.get_result_async()
        result_future.add_done_callback(lambda done, state=arm: self._on_goal_result(state, done))

    def _on_goal_result(self, arm: ArmState, future):
        arm.goal_active = False
        self.move_group_busy = False
        try:
            result = future.result()
        except Exception as exc:
            self.get_logger().warn(f"{arm.config.name} MoveGroup result failed: {exc}")
            return
        code = result.result.error_code.val if result is not None else 99999
        if code == 1:
            self.get_logger().info(f"{arm.config.name} MoveGroup goal succeeded")
        else:
            self.get_logger().warn(f"{arm.config.name} MoveGroup goal failed: error_code={code}")
