...
OSAL_PACKED_BEGIN // 以防编译器优化导致字节不对齐，从而导致通信错误
...
{
   ecx_initmbxpool(context);  // 初始化邮箱池
   return ecx_setupnic(&context->port, ifname, FALSE); // 设置网络接口，非冗余模式
}
 ...
 * - 准备冗余操作所需的广播帧
 * 
 * 冗余模式用于提高可靠性，当主网卡故障时自动切换到备用网卡
 * 
 * @param[in]  context  上下文结构体指针
 * @param[in]  redport  指向冗余端口数据的指针
 * @param[in]  ifname   主网络接口名称，例如 "eth0"
 * @param[in]  if2name  备用网络接口名称，例如 "eth1"
 * @return >0 表示成功，<=0 表示失败
 */
int ecx_init_redundant(ecx_contextt *context, ecx_redportt *redport, const char *ifname, char *if2name)
{
   ...
   ecx_initmbxpool(context);              // 初始化邮箱池
   context->port.redport = redport;      // 设置冗余端口
   ecx_setupnic(&context->port, ifname, FALSE); // 设置主网络接口
   rval = ecx_setupnic(&context->port, if2name, TRUE); // 设置备用网络接口
   /* 为冗余操作准备 "dummy" BRD 发送帧 */
 ...
 * - 关闭网络接口
 * 
 * 注意：这些互斥锁是在 ecx_config_init() 中创建的，必须在关闭套接字之前销毁
 * 
 * @param[in]  context 上下文结构体指针
 */
void ecx_close(ecx_contextt *context)
{
   /* 这些互斥锁是在 ecx_config_init() 中创建的，应该在关闭套接字之前销毁 */
   for (int lp = 0; lp < EC_MAXGROUP; lp++) // 遍历所有组
   {
      ec_mbxqueuet *mbxqueue = &(context->grouplist[lp].mbxtxqueue);
      if (mbxqueue->mbxmutex)
         osal_mutex_destroy(mbxqueue->mbxmutex); // 销毁互斥锁
   }

   osal_mutex_destroy(context->mbxpool.mbxmutex); // 销毁邮箱池互斥锁
   ecx_closenic(&context->port); // 关闭网络接口
}
 ...
 * 功能：
 * - 将目标状态写入从站的 AL 控制寄存器
 * - 从站 0 是一个虚拟从站，用于广播到所有从站
 * 
 * 注意：此函数不检查实际状态是否已改变
 * 
 * @param[in]  context 上下文结构体指针
 * @param[in] slave    从站编号，0 表示主站（广播到所有从站）
 * @return 工作计数器或 EC_NOFRAME
 */
...
/**
 * 阻塞等待从站进入指定状态 (INIT/PRE-OP/SAFE-OP/OP/BOOT)。
 *
 * 做法：每 1ms 轮询一次 AL Status 寄存器 (0x0130)，直到状态匹配或超时。
 *
 * 限制：
 * - slave=0 表示所有从站，用 BRD 广播读，返回值是所有从站状态的按位或；
 *   因此不能用 slave=0 + reqstate=BOOT (BOOT 与 INIT|PRE_OP 位冲突)。
 * - 本函数只查一个目标，要刷新整张 slavelist 用 ecx_readstate()。
 *
 * @param context  上下文
 * @param slave    从站号，0=所有从站 (广播)
 * @param reqstate 期望状态
 * @param timeout  超时 (微秒)
 * @return 实际读到的状态 (成功时 == reqstate；超时则是当时实际状态)
 */
uint16 ecx_statecheck(ecx_contextt *context, uint16 slave, uint16 reqstate, int timeout)
{
   ...
   do
   {
      if (slave < 1)
      {
         /* slave=0：广播读 AL Status，rval 是总线上所有从站状态的按位或 */
         rval = 0;
      ...
      state = rval & 0x000f; /* AL Status 低 4 位才是状态值，高位是错误标志 */
      if (state != reqstate)
      {
         osal_usleep(1000); /* 还没到，歇 1ms 再轮询 */
      }
   } while ((state != reqstate) && (osal_timer_is_expired(&timer) == FALSE));
   /* 保存最终读到的完整 AL Status (含错误标志位) */
   context->slavelist[slave].state = rval;
 ...
 * 功能：
 * - 将输出数据发送给指定组的所有从站
 * - 如果过程数据无法放入一个数据报，则使用多个数据报
 * - 使用栈来重组从站的响应
 * - 支持分布式时钟（DC）同步
 * 
 * 数据传输策略：
 * - 如果 LRW（逻辑读写）被阻塞，则使用 LRD（逻辑读）和 LWR（逻辑写）分别处理输入和输出
 * - 如果 LRW 可用，则使用 LRW 同时读写
 * - 支持分段传输（segment transfer）以处理大块数据
 * 
 * 在重叠模式下，输出数据在发送帧中发送，输入数据将在接收帧中替换输出数据
 * 
 * @param[in]  context 上下文结构体指针
 * @param[in]  group   组号
 * @return >0 表示过程数据已传输
 ...
 * 从从站接收过程数据（按组）
 * 功能：
 * - 这是 ecx_send_processdata() 的第二部分
 * - 使用栈的帮助将接收到的数据报与过程数据重组
 ...
 * 1. 从栈中获取发送时保存的索引
 * 2. 等待接收对应的数据帧
 * 3. 根据命令类型（LRD/LRW/LWR）处理数据
 * 4. 提取工作计数器（WKC）和 DC 时间
 * 5. 将输入数据复制到过程数据缓冲区
 * 
 * @param[in]  context 上下文结构体指针
 * @param[in]  group   组号
 * @param[in]  timeout 超时时间（微秒）
 * @return 工作计数器（Work Counter），如果超时返回 EC_NOFRAME
 */
...
/**
 * 发送过程数据到从站
 * 这是一个封装函数，将 group 参数默认设置为 0（默认组）
 * 作用：简化 API 调用，用户不需要每次都指定 group 参数
 * @param[in]  context        上下文结构体
 * @return 工作计数器（Work Counter）
 */
