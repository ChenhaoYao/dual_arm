...
OSAL_THREAD_FUNC_RT ecatthread(void)
{
   ...
   for (;;)
   {
      ...
      if (run_axes)
      {
         ...
         /* sync_trigger：所有轴 Op enabled 后等 5 周期再启动 CST */
         int all_enabled = 1;
         for (int a = 0; a < motion_axis_count; a++)
         ...
         for (int a = 0; a < motion_axis_count; a++)
            axis_step(&g_axes[a], sync_trigger);
      }
...
static void axis_step(axis_state_t *axis, int sync_trigger)
{
   ...
   else if ((sw & 0x006F) == 0x0027)
   {
      ...
      if (!axis->enabled_logged)
      {
         ...
         printf("[AXIS%d] cycle=%d Op Enabled，等待 %dms 后发转矩指令\n",
                axis->slave, cycle, ENABLE_WAIT_MS);
      }

      /* 使能后等待 ENABLE_WAIT_CYCLES，期间 CW=0x000F、转矩=0 */
      ...
      {
         /* sync_trigger 前：力矩保持 0 */
         tgt_torq = 0;
         if (sync_trigger)
         {
            axis->cst_started = 1;
            axis->sync_cycle  = cycle;
            printf("[AXIS%d] cycle=%d 同步启动 CST 力矩斜坡\n", axis->slave, cycle);
         }
      }
      else
      {
         ...
         else
         {
            ...
            if (!axis->finished_logged)
            {
               ...
               printf("[AXIS%d] cycle=%d CST 力矩斜坡完成，pos=%d vel=%d torq=%d\n",
                      axis->slave, cycle, pos, vel, act_torq);
            }
         }
      }
   }
   ...
   if (sw != axis->prev_sw || er != axis->prev_err)
   {
      printf("[CHG][AXIS%d] cycle=%d SW:0x%04X→0x%04X (%s) Err:0x%04X→0x%04X "
             "CW=0x%04X ModeDisp=%d pos=%d vel=%d torq=%d tgt_torq=%d\n",
             axis->slave, cycle, axis->prev_sw, sw, cia402_state_name(sw),
             axis->prev_err, er, cw, mode_disp, pos, vel, act_torq, tgt_torq);
      axis->prev_sw  = sw;
      axis->prev_err = er;
   }
   ...
   if ((cycle % LOG_DIV) == 0)
   {
      printf("[TICK][AXIS%d] cycle=%d wkc=%d SW=0x%04X (%s) CW=0x%04X ModeDisp=%d "
             "pos=%d vel=%d torq=%d tgt_torq=%d Err=0x%04X\n",
             axis->slave, cycle, wkc, sw, cia402_state_name(sw), cw, mode_disp,
             pos, vel, act_torq, tgt_torq, er);
   }
...
{
   setup_log();
   printf("SOEM cst_test — CST 模式力矩控制，目标 %d‰\n", TARGET_TORQUE_PERMILLE);
